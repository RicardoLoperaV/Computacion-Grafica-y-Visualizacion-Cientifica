...
def plot_3d_analysis(node_file, ele_file, sol_file, title_prefix):
    # 1. Load Data
    ...
    ax.set_title(f"{title_prefix} - 3D Material Domains")
...
if __name__ == "__main__":
    ...
    def run_case_3d(node, ele, sol, title):
        print(f"\n--- Processing {title} (3D) ---")
        ...
        if real_node and real_ele:
            plot_3d_analysis(real_node, real_ele, real_sol, title)
        else:
            print("Missing files.")

    # 1. L-DOMAIN (Usually 3D L-shape)
    run_case_3d('Ldomain.1.node', 'Ldomain.1.ele', None, 'L-Domain')

    # 2. BENCHMARK CASE 1
    run_case_3d('ExBenchmark.1.node', 'ExBenchmark.1.ele', 'solucionAVpsi.1.txt', 'Benchmark Case 1')

    # 3. BENCHMARK CASE 5
    run_case_3d('ExBenchmark.5.node', 'ExBenchmark.5.ele', 'solucionAVpsi.5.txt', 'Benchmark Case 5')